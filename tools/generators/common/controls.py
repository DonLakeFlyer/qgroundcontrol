...
def render_label(
    ...
    *,
    ...
    lines.append(f'{indent}    text: qsTr("{text}")')
...
def render_slider(
    ...
    *,
    ...
    label_line = f'    label: qsTr("{label}")' if label else "    label: fact.label"
    inner_indent = indent
    ...
    if has_button:
        ...
        lines.append(f'{inner_indent}    text: qsTr("{button.text}")')
...
def render_checkbox(
    ...
    *,
    ...
    label_line = f'    {label_property}: qsTr("{label}")' if label else f"    {label_property}: {label_source}"
    lines = [
        f"{indent}{qml_type} {{",
...
def render_combobox(
    ...
    *,
    ...
    if qml_type == "LabelledFactComboBox":
        label_line = f'    label: qsTr("{label}")' if label else f"    label: {label_source}"
    else:
        label_line = None
...
def render_textfield(
    ...
    *,
    ...
    if enable_when:
        lines.append(f"{indent}    enabled: {enable_when}")
    if placeholder:
        lines.append(f'{indent}    textField.placeholderText: qsTr("{placeholder}")')
    if extra_lines:
...
def render_radiogroup(
    ...
    *,
    ...
    if label:
        ...
        lines.append(f'{indent}    text: qsTr("{label}")')
        lines.append(f'{indent}}}')
    inner = indent + "    "
    ...
    for opt in options:
        ...
        lines.append(f'{inner}    text: qsTr("{opt.label}")')
        if opt.checked:
...
def render_dialog_button(
    ...
    *,
    ...
    lines.append(f'{indent}    text: qsTr("{dialog_button.text}")')
...
def render_action_button(
    ...
    *,
    ...
    lines: list[str] = []
    lines.append(f"{indent}QGCButton {{")
    lines.append(f'{indent}    text: qsTr("{action_button.text}")')
