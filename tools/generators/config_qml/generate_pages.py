#!/usr/bin/env python3
"""Generate QML vehicle config pages from ConfigUI.json definitions.

Usage:
    python -m tools.generators.config_qml.generate_pages \\
        --pages-dir src/AutoPilotPlugins/PX4/pages \\
        --output-dir build/src/AutoPilotPlugins/PX4/generated

Each ``<Name>.ConfigUI.json`` produces ``<Name>Component.qml``.
"""
...
def _output_name(json_name: str) -> str:
    ...
    ``Safety.ConfigUI.json`` -> ``SafetyComponent.qml``
    """
    stem = json_name.removesuffix(".ConfigUI.json")
...
def main() -> None:
    ...
    parser.add_argument("--pages-dir", type=Path, required=True,
                        help="Directory containing *.ConfigUI.json files")
    ...
    json_files = sorted(args.pages_dir.glob("*.ConfigUI.json"))
    if not json_files:
        print(f"Warning: no *.ConfigUI.json files found in {args.pages_dir}", file=sys.stderr)
        return
