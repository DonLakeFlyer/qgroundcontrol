...
VehicleComponent *findComponent(Vehicle *vehicle, const QString &name)
{
...
{
    QQuickItem *compBtn = findVisibleItem(_rootItem, compObjectName, 3000);
    QVERIFY2(compBtn, qPrintable(QStringLiteral("Component button not found: %1").arg(compObjectName)));

    // The property drives the visual: wait for it to settle first
    QVERIFY2(QTest::qWaitFor([&] { return compBtn->property("setupComplete").toBool() == expectSetupComplete; }, 5000),
...
void APMFreshFlashUITest::_testFreshFlashSetupState()
{
    ...
    // -------------------------------------------------------------------
    VehicleComponent *frameComp = findComponent(vehicle, QStringLiteral("Frame"));
    VehicleComponent *sensorsComp = findComponent(vehicle, QStringLiteral("Sensors"));
    VehicleComponent *radioComp = findComponent(vehicle, QStringLiteral("Radio"));
