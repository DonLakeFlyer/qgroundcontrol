...
void SurfaceModel::update()
{
    ...
    // Rects of desired patches not yet resident: a no-longer-desired patch
    // overlapping one may not be removed yet, or the surface would show a
    // hole until the capped adds catch up
    ...
    for (const TileMath::TileKey& key : desired) {
        ...
        PatchData data;
        data.maxHeight = _samplePatchMaxHeight(key);
...
QList<SurfaceModel::Patch> SurfaceModel::patches() const
{
    ...
    for (auto it = _patches.cbegin(); it != _patches.cend(); ++it) {
        result.append(Patch{it.key(), _field->samplePatch(it.key(), kGridSize), true, false});
    }
    return result;
}
...
std::optional<SurfaceModel::Patch> SurfaceModel::patch(const TileMath::TileKey& key) const
{
    ...
    return Patch{key, _field->samplePatch(key, kGridSize), true, false};
}
...
float SurfaceModel::_samplePatchMaxHeight(const TileMath::TileKey& key) const
{
    float maxHeight = 0.0f;
    for (const float height : _field->samplePatch(key, kGridSize)) {
...
void SurfaceModel::_fieldRegionChanged(const QRectF& worldRect)
{
    ...
    for (auto it = _patches.begin(); it != _patches.end(); ++it) {
        ...
        it.value().maxHeight = _samplePatchMaxHeight(it.key());
