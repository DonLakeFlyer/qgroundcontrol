...
SurveyComplexItem::SurveyComplexItem(Vehicle* vehicle, const QString& kmlOrShpFile, QObject* parent)
    ...
    , _entryPoint               (EntryLocationTopLeft)
{
    ...
    if (_vehicle->multiRotor() && _turnAroundDistanceFact.rawValue().toDouble() == _turnAroundDistanceFact.rawDefaultValue().toDouble()) {
